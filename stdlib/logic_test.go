...
func TestNot(t *testing.T) {
	...
	if !r1.Equiv(e.Boolean(false)) { t.Error("(not #t) should be #f") }
	r2, _ := evalWithStdlib("(not #f)")
	if !r2.Equiv(e.Boolean(true)) { t.Error("(not #f) should be #t") }
}
...
func TestAnd(t *testing.T) {
	...
	if !r1.Equiv(e.Boolean(true)) { t.Error("(and #t #t) should be #t") }
	r2, _ := evalWithStdlib("(and #t #f)")
	if !r2.Equiv(e.Boolean(false)) { t.Error("(and #t #f) should be #f") }
}
