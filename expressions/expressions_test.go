...
func TestForeignRepresentation(t *testing.T) {
	cases := []struct {
		...
		{Wrap(testA), "#<foreign:expressions.testStructA{value:\"a\"}>"},
		{Wrap(testStructA{"b"}), "#<foreign:expressions.testStructA{value:\"b\"}>"},
		{Wrap(testB), "#<foreign:expressions.testStructB{num:23}>"},
		{Wrap(testC), "#<foreign:expressions.testStructC{something:interface {}(nil)}>"},
		{Wrap(&testA), "#<foreign:&expressions.testStructA{value:\"a\"}>"},
		{Wrap(&testStructA{"b"}), "#<foreign:&expressions.testStructA{value:\"b\"}>"},
		{Wrap(&testB), "#<foreign:&expressions.testStructB{num:23}>"},
		{Wrap(&testC), "#<foreign:&expressions.testStructC{something:interface {}(nil)}>"},
		{Wrap(nil), "#<foreign:<nil>>"},
		{Wrap([]testStructA{testStructA{"a"}}), "#<foreign:[]expressions.testStructA{expressions.testStructA{value:\"a\"}}>"},
	}
...
var equivCases = []struct {
	...
	{Wrap(testA), Wrap(testA), true},
	{*Wrap(testA), Wrap(testA), true},
	{Wrap(testA), *Wrap(testA), true},
	{*Wrap(testA), *Wrap(testA), true},
	{Wrap(testB), Wrap(testA), false},
	{Wrap(testB), Wrap(testStructB{23}), true},
	{Wrap(testB), Wrap(testStructB{99}), false},
	{Wrap(testStructC{testB}), Wrap(testStructC{testStructB{23}}), true},
	{Wrap(testStructC{&testB}), Wrap(testStructC{&testB}), true},
	{Wrap(testStructC{&testB}), Wrap(testStructC{&testStructB{23}}), false},
	{Wrap(&testStructC{testB}), Wrap(&testStructC{testB}), false},
	{Wrap(testStructC{testB}), Wrap(testStructC{&testStructB{23}}), false},
	{*Wrap(testStructC{testB}), Wrap(testStructC{testStructB{23}}), true},
	{*Wrap(testStructC{&testB}), Wrap(testStructC{&testB}), true},
	{*Wrap(testStructC{&testB}), Wrap(testStructC{&testStructB{23}}), false},
