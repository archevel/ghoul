...
func TestForeignRepresentation(t *testing.T) {
	cases := []struct {
		...
		{Wrap(testA), "#<foreign:bones.testStructA{value:\"a\"}>"},
		{Wrap(testStructA{"b"}), "#<foreign:bones.testStructA{value:\"b\"}>"},
		{Wrap(testB), "#<foreign:bones.testStructB{num:23}>"},
		{Wrap(testC), "#<foreign:bones.testStructC{something:interface {}(nil)}>"},
		{Wrap(&testA), "#<foreign:&bones.testStructA{value:\"a\"}>"},
		{Wrap(&testStructA{"b"}), "#<foreign:&bones.testStructA{value:\"b\"}>"},
		{Wrap(&testB), "#<foreign:&bones.testStructB{num:23}>"},
		{Wrap(&testC), "#<foreign:&bones.testStructC{something:interface {}(nil)}>"},
		{Wrap(nil), "#<foreign:<nil>>"},
		{Wrap([]testStructA{testStructA{"a"}}), "#<foreign:[]bones.testStructA{bones.testStructA{value:\"a\"}}>"},
	}
